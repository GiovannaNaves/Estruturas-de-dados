...
class Huffman {
    ...
    void WriteHuffmanCode(const std::string& text, const std::string huffmanCode[], FILE* f) {
                ...
                for (char ch : text) {
                    string code = huffmanCode[(int) ch];
    ...
    void buildHuffmanTree(string inputFilename, string keyFilename, string outputFilename, char action)
    {
        if (action == 'c') {
            ofstream chave(keyFilename);
        ...
        if (action == 'd'){
            ...
            while(getline(keyFile, line, '\n')){
                ...
                string letter, numberOcurrences; 
                ss >> letter;
                ss >> numberOcurrences;
                freq[stoi(letter)] = stoi(numberOcurrences.c_str());
            }
            int nodeCount = 0;
            // Create an array to store live nodes of Huffman tree;
            Node* pq[MAX_CHAR] = { nullptr };
            ...
            if (outputFile.is_open()) {
                ...
                outputFile.close();
                cout << "A saída foi escrita no arquivo " << outputFilename << endl;
            }
            else {
                cout << "Houve um erro na abertura do arquivo " << outputFilename << endl;
            }
